      ...
      `UPDATE mail_candidates
       ...
       WHERE user_id = ? AND verdict = 'pending' AND user_locked = 0
         AND (lower(from_addr) LIKE '%no-reply%'
           ...
           OR snippet LIKE '%automatically generated by Gmail%')`
    )
  ...
  try {
    ...
    for (const ref of fresh) {
      ...
      const bulk =
        ...
        // Gmail's own service mail (forwarding-verification codes etc.) comes
        // from the user's OWN address with no bulk headers, but Gmail stamps
        // this exact snippet on all of it. Nobody can reply to it.
        (meta.snippet ?? "").includes("automatically generated by Gmail");
